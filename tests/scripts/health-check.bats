...
@test "empty worktrees dir returns empty summary" {
  mkdir -p .worktrees
  run bash "$SCRIPTS_DIR/health-check.sh" --worktrees-dir ".worktrees"
  [ "$status" -eq 0 ]
  [[ "$output" == *"SUMMARY:0:0:0:0:0:0"* ]]
}
...
@test "clean worktree with done status classified as CLEAN_DONE" {
  # Create a worktree
  git worktree add .worktrees/story-1 -b story/story-1 >/dev/null 2>&1
...
@test "worktree with uncommitted changes classified as DIRTY" {
  git worktree add .worktrees/story-2 -b story/story-2 >/dev/null 2>&1
  echo "dirty content" > .worktrees/story-2/dirty-file.txt
...
@test "worktree with commits ahead classified as COMMITTED" {
  git worktree add .worktrees/story-3 -b story/story-3 >/dev/null 2>&1
  cd .worktrees/story-3
...
@test "worktree with no commits ahead classified as STALE" {
  git worktree add .worktrees/story-4 -b story/story-4 >/dev/null 2>&1
...
@test "non-worktree directory classified as DIRTY (inherits parent git context)" {
  ...
  mkdir -p .worktrees/orphan-dir
  echo "not a git repo" > .worktrees/orphan-dir/file.txt
...
@test "detached HEAD classified as ORPHAN" {
  git worktree add .worktrees/detached -b temp-branch >/dev/null 2>&1
...
@test "mixed worktree statuses produce correct summary" {
  # STALE worktree
  git worktree add .worktrees/stale-one -b story/stale-one >/dev/null 2>&1

  # DIRTY worktree
  git worktree add .worktrees/dirty-one -b story/dirty-one >/dev/null 2>&1
  echo "dirty" > .worktrees/dirty-one/x.txt
...
@test "custom base branch works" {
  ...
  git worktree add .worktrees/story-x -b story/story-x >/dev/null 2>&1
